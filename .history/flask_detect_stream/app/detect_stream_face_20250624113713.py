...
def run_detection_face_food(video_source=0):
    ...
    while cap.isOpened():
        ...
        if waste_flag and has_face:
            print(f"[DEBUG] 当前识别：{safe_name}，上次记录时间：{last_capture_time[safe_name]}，当前时间：{now}")
            if now - last_capture_time[safe_name] > 60:
                last_capture_time[safe_name] = now
                timestamp = time.strftime("%Y%m%d_%H%M%S")
                filename = f"waste_captures/{safe_name}_{ratio:.2f}_{timestamp}.jpg"
                cv2.imwrite(filename, annotated)
                print(f"[SAVE] {filename}")
            else:
                print(f"[SKIP] {safe_name} 截图频率受限，未保存")
